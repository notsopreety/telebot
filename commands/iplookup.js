...
module.exports.run = async function ({ bot, args, chatId }) {
    // Extract the IP address from arguments
    const ipAddress = args[0];

    // Check if an IP address is provided
    if (!ipAddress) {
        bot.sendMessage(chatId, "Usage: /ipinfo [IP Address]");
        return;
    }

    // Send a pre-processing message
    const preMessage = await bot.sendMessage(chatId, "Fetching IP information...");

    try {
        const response = await axios.get(`https://api.ipgeolocation.io/ipgeo?apiKey=84ee8ffc30644f58a7878d83519ae4dc&ip=${ipAddress}`);
...
🌐 𝗚𝗢𝗢𝗚𝗟𝗘 𝗠𝗔𝗣\n[Open in Google Maps](https://www.google.com/maps?q=${data.latitude},${data.longitude})`;
            ...
            bot.sendMessage(chatId, "An error occurred while fetching IP information.");
        }
    } catch (error) {
        console.error(error);
        bot.sendMessage(chatId, "An error occurred while fetching IP information.");
